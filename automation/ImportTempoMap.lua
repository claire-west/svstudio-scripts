--[[ ImportTempoMap.lua
    Reads a MIDI file (default TempoMap.mid in the SVP location) and applies any tempo changes and time signatures to the current project.
    While it is normally only possible to create tempo markers at the start of the quarter ("on the beat"), this script is capable of adding tempo markers to other positions as dictated by the MIDI file.
    Gradual tempo changes are not supported by the MIDI file standard, and may cause odd results depending on how your DAW encodes them, if it does at all.
--]]
SCRIPT_TITLE = "Apply Tempo from MIDI"

-- if a file with this name is present in the SVP location, it will be used without prompting for input
local MIDI_FILE_NAME = 'TempoMap.mid'

-- if TempoMap.mid is not found, this value will be shown in the text box by default
local DEFAULT_FILE_PATH = ''

-- set this to false if you don't want to also read time signatures from the MIDI file
local PROCESS_TIME_SIGNATURES = true
...
local DEBUG = false
local DEBUG_EVENTS = 'MIDI events:'
...
function handlers.setTempo(tempo)
    if DEBUG then DEBUG_EVENTS = DEBUG_EVENTS..'\nsetTempo '..tempo end
    table.insert(tempoMarkers, {
        position = ticksToBlicks(ticks),
        tempo = tempo
    })
end
function handlers.timeSignature(numerator, denominator)
    if DEBUG then DEBUG_EVENTS = DEBUG_EVENTS..'\ntimeSignature '..numerator..'/'..denominator end
...
function main()
    processMidiFile(getMidiReader())
    if DEBUG then SV:showMessageBox('Debug Output', DEBUG_EVENTS) end
    SV:finish()
end

function processMidiFile(MidiReader)
    ...
    if not pcall(function() MidiReader.process(file, callback, false, 1) end) then
    ...
    if #tempoMarkers > 50 and not SV:showOkCancelBox(SV:T(SCRIPT_TITLE), 'The selected MIDI file has '..#tempoMarkers..' tempo changes.\n\nA large number of tempo markers may produce odd results, especially if they are close together.\n\nContinue anyway?') then
      return
    end

    if PROCESS_TIME_SIGNATURES and #timeSignatures > 0 then
        -- remove existing time signatures markers
        local existingTimeSignatures = timeAxis:getAllMeasureMarks()
        for i = 1, #existingTimeSignatures do
            timeAxis:removeMeasureMark(existingTimeSignatures[i].position)
        end

        -- add time signature markers found in the MIDI track
        for i = 1, #timeSignatures do
            local timeSignature = timeSignatures[i]
            local measure = timeAxis:getMeasureAt(timeSignature.position)
            timeAxis:addMeasureMark(measure, timeSignature.numerator, timeSignature.denominator)
        end
    end
...
-- The module was embedded in this script to avoid requiring users to download a separate dependency --
-------------------------------------------------------------------------------------------------------
